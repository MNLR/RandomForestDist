...
itemp.gammaLLMME <- function(y, offset, parms, wt) {   # Initialization function
  ...
  sfun <- function(yval, dev, wt, ylevel, digits ) {
    paste(" shape = ", format(signif(yval[1], digits)), # shape/rate equals mean
          " rate = ",  format(signif(yval[2], digits)),
...
etemp.gammaLLMME <- function(y, wt, parms) {  # Evaluation function
  ...
  label <- c(shape = shape, rate = rate)
  # print(paste0("Node with ", length(y), " elements and mean ", my, "..."))
  # print(paste0("Label: ", label, collapse = ", "))
  # print(paste0("Deviance: ", - sum(dgamma(x = y, shape = shape, rate = rate,
  #                                       log = T))))
return(list(label = label,
            deviance = - sum(dgamma(x = y, shape = shape, rate = rate,
                                    log = T)))
)
}

stemp.gammaLLMME <- function(y, wt, x, parms, continuous)   # Split Function, higher goodness is better. <= 0 is no improvement
...

{
  ...
  if (continuous) {
    ...
    # print(paste0("Which Goodness Improvement Sorted Descending (mean =", my , "):"))
...
gammaLLMME <- function() list(eval = etemp.gammaLLMME,
                               split = stemp.gammaLLMME,
                               init = itemp.gammaLLMME)
